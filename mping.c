   ...
   if (iface)
   {
      ...
      if (!inet_aton (dotted, &localIP))
      {
         fprintf (stderr, "Failed converting IP address %u.%u.%u.%u --> 0x%X of iface:%s.\n"
                  "Error %d: %s\n", addr[0], addr[1], addr[2], addr[3], localIP.s_addr,
                  iface, errno, strerror (errno));
         exit (1);
      }
...
void receiver_listen_loop ()
{
   ...
   while (1)
   {
      ...
      if (process_mping_packet (recv_packet, recv_len, SENDER) == 0)
      {
         ...
         resp_buf[x] = (struct response_buffer *)
            malloc (sizeof (struct response_buffer));
         ...
         resp_buf[x]->send_time.tv_usec +=
            (long)((interval - (long)interval) * 1000000.0);
...
int process_mping_packet (char *packet, int recv_len, unsigned char type)
{
   ...
   {
      if (verbose)
         printf ("Discarding packet: too small (%zd bytes)\n", strlen (packet));
